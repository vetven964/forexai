...
function candleGuard(snapshot){
  ...
  for(const tf of required){
    ...
    frames[tf]={raw:raw.length,valid:valid.length,ready:valid.length>=25};
  }
...
function scoreSnapshot(s){
  ...
  const total=Object.keys(gates).length,passed=Object.values(gates).filter(Boolean).length,rawScore=passed/total*100,signed=side==='BUY'?tier(rawScore):side==='SELL'?-tier(rawScore):0,signal=signed>0?'BuyBullish':signed<0?'SellBearish':'WAIT';
  const authorized=Math.abs(signed)>=75&&Object.values(gates).every(Boolean)&&raw.length>=25&&rows.length===4&&s.connected===true&&guard.ready;
  ...
  return{symbol:'XAUUSD',price,side,signal,score:signed,scoreTier:Math.abs(signed),bias:side==='BUY'?'Bullish':side==='SELL'?'Bearish':'Neutral',timeframe:'M15',realtime:s.connected===true,mtf:rows,candleGuard:guard,gates,gateCount:`${passed}/${total}`,authorized,entry,sl,tp,riskZones,generatedAt:new Date().toISOString(),reason:authorized?'All equal-weight ICT/CRT confirmations passed':'WAIT — one or more equal-weight confirmation gates are incomplete'};
}
function formatTelegram(a){
  const action=a.signal==='BuyBullish'?'🟢 BUY LONG / BULLISH':a.signal==='SellBearish'?'🔴 SELL SHORT / BEARISH':'🟡 WAIT',zone=a.authorized?'ENTER ZONE':'NO ENTRY ZONE',f=v=>v==null?'WAIT':Number(v).toFixed(2),rz=a.riskZones||{},protection=a.authorized?(a.side==='BUY'?`🛡️ RS/SP PROTECTED · Support *${f(rz.support)}* → SL *${f(a.sl)}*`:`🛡️ RS/SP PROTECTED · Resistance *${f(rz.resistance)}* → SL *${f(a.sl)}*`):'🛡️ RS/SP: WAIT · SL protection not authorized';
  if(!a.authorized)return['🤖 *V-Zone AI*','',`📊 XAUUSD | ${a.timeframe} REALTIME`,`💰 Price: *${f(a.price)}*`,`⚡ Action: *${action}*`,`📍 *${zone}*`,`📈 Score: *${a.score}/100* | Bias: *${a.bias}*`,`🧠 Equal-weight gates: *${a.gateCount}*`,'',`🕯️ Candle Guard: *${a.candleGuard?.ready?'PASS — CLOSED':'WAIT — FAKE/ACTIVE CANDLE REJECTED'}*`,'','⏳ *WAIT — NO TRADE ALERT*','🔒 ICT + CRT + MTF + execution gates not fully confirmed.',`🕒 ${a.generatedAt}`].join('\n');
  return['🤖 *V-Zone AI*','',`📊 XAUUSD | ${a.timeframe} REALTIME`,`💰 Price: *${f(a.price)}*`,`⚡ *${action}*`,`📍 *ENTER ZONE*`,`📈 Score: *${a.score}/100* | Bias: *${a.bias}*`,'',`🎯 Entry: *${f(a.entry)}*`,`🛑 SL: *${f(a.sl)}*`,`🎯 TP1: *${f(a.tp[0])}*`,`🎯 TP2: *${f(a.tp[1])}*`,`🎯 TP3: *${f(a.tp[2])}*`,'',`${protection}`,'🕯️ Candle Guard: *PASS — CLOSED CANDLE*','🚫 Fake candle: *REJECTED*','🧠 ICT + CRT + MTF: *CONFIRMED / EQUAL-WEIGHT*','🔐 *READY — SIGNAL AUTHORIZATION PASSED*',`🕒 ${a.generatedAt}`].join('\n');
}
module.exports={TIERS,bars,candleGuard,scoreSnapshot,formatTelegram};
