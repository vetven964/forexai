...
async function scan(force=false){try{const snapshot=await market();const a=scoreSnapshot(snapshot);console.log(`[V-ZONE TELEGRAM] SCAN | signal=${a.signal} | bias=${a.bias} | score=${a.score} | gates=${a.gateCount} | authorized=${a.authorized}`);if(!a.authorized){if(force)console.log('[V-ZONE TELEGRAM] WAIT | no authorized entry');return;}const k=key(a);if(!force&&k===lastKey)return;await bot.sendMessage(CHAT_ID,formatTelegram(a),{parse_mode:'Markdown'});lastKey=k;console.log(`[V-ZONE TELEGRAM] SENT | signal=${a.signal} | authorized=true`);}catch(e){console.warn('[V-ZONE TELEGRAM] DELIVERY ERROR |',e.message);}}
...
console.log('[V-ZONE TELEGRAM] ONLINE | XAUUSD REALTIME | ICT+CRT | FAIL-CLOSED');
