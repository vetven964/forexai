...
async function market(){try{return normalizeMarket(await requestJson('/api/telegram/market-snapshot'));}catch(primary){try{return normalizeMarket(await requestJson('/api/telegram/market-snapshot-v5'));}catch(canonical){throw new Error(`MARKET_BRIDGE_FAILED | primary=${primary.message} | canonical=${canonical.message}`);}}}
