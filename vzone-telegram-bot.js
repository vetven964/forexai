...
const ENGINE_VERSION='V-ZONE-TELEGRAM-V2';
if(!TOKEN||!CHAT_ID){console.error('[V-ZONE TELEGRAM] CONFIG ERROR | TELEGRAM_TOKEN/CHAT_ID missing');process.exit(1);}
...
async function market(){const headers=BRIDGE_KEY?{'X-VTRADE-TELEGRAM-KEY':BRIDGE_KEY}:{};const r=await fetch(CORE_URL+'/api/telegram/market-snapshot',{headers,cache:'no-store'});const text=await r.text();let d={};try{d=JSON.parse(text);}catch{throw new Error(`CORE_NOT_JSON HTTP ${r.status}`);}if(!r.ok)throw new Error(d?.error||`HTTP ${r.status}`);return normalizeMarket(d);}
function key(a){return [a.signal,a.score,a.bias,a.entry,a.sl,...(a.tp||[])].join('|');}
async function scan(force=false){try{const snapshot=await market();const a=scoreSnapshot(snapshot);console.log(`[V-ZONE TELEGRAM] SCAN | engine=${ENGINE_VERSION} | signal=${a.signal} | bias=${a.bias} | score=${a.score} | gates=${a.gateCount} | authorized=${a.authorized}`);if(!a.authorized&&!force)return;const k=key(a);if(!force&&k===lastKey)return;await bot.sendMessage(CHAT_ID,formatTelegram(a),{parse_mode:'Markdown'});if(a.authorized)lastKey=k;console.log(`[V-ZONE TELEGRAM] SENT | engine=${ENGINE_VERSION} | authorized=${a.authorized}`);}catch(e){console.warn('[V-ZONE TELEGRAM] DELIVERY ERROR |',e.message);}}
bot.onText(/^\/(signal|scan)(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;await scan(true);});
bot.onText(/^\/(status|start)(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;await bot.sendMessage(msg.chat.id,'🤖 *V-Zone AI*\n🟢 Telegram Engine: ONLINE\n📊 XAUUSD: REALTIME\n🧠 ICT + CRT: EQUAL-WEIGHT\n🔐 Auto Order: FAIL-CLOSED\n⚙️ Engine: *V-ZONE-TELEGRAM-V2*',{parse_mode:'Markdown'});});
bot.on('polling_error',e=>console.warn('[V-ZONE TELEGRAM] POLLING ERROR |',e.message));
console.log(`[V-ZONE TELEGRAM] ONLINE | ${ENGINE_VERSION} | XAUUSD REALTIME | ICT+CRT | FAIL-CLOSED`);
