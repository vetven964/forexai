...
const ENGINE_VERSION='V-ZONE-TELEGRAM-V5';
const MARKET_PATHS=['/api/pre-market/mt5-authoritative','/api/analysis/xauusd','/api/market-snapshot','/api/telegram/market-snapshot'];
...
function normalizeMarket(d){const candidates=[d?.snapshot,d?.marketSnapshot,d?.market,d?.data,d?.result,d];const s=candidates.find(x=>x&&typeof x==='object'&&(x.timeframes||x.connected!==undefined||x.price!==undefined||x.state!==undefined));if(!s)throw new Error('market snapshot missing');const tf=s.timeframes||s.mtf||{};return {...s,connected:s.connected===true||s.state==='READY'||s.mt5?.state==='READY',price:s.price??s.livePrice??s.currentPrice??s.quote?.price??s.quote?.bid??s.quote?.ask,timeframes:tf};}
...
async function market(){let lastError=null;for(const marketPath of MARKET_PATHS){try{const d=await requestJson(marketPath);const normalized=normalizeMarket(d);console.log(`[V-ZONE TELEGRAM] MARKET BRIDGE OK | ${marketPath}`);return normalized;}catch(e){lastError=e;console.warn(`[V-ZONE TELEGRAM] MARKET ENDPOINT MISS | ${marketPath} | ${e.message}`);}}throw new Error(`MARKET_BRIDGE_FAILED | ${lastError?.message||'no market endpoint available'}`);}
