// V-ZONE AI New Web App route — runtime-safe, idempotent.
'use strict';
const fs=require('fs');const path=require('path');
const ROOT=__dirname;const SERVER=path.join(ROOT,'server.js');const UI=path.join(ROOT,'vtrade-new');
...
function install(){
 ...
 src=src.replace(anchor,anchor+code,1);fs.writeFileSync(SERVER,src,'utf8');
 console.log('[V-ZONE UI] server.js route installed');
}
try{install();module.exports={install};}catch(e){console.error('[V-ZONE UI] INSTALL ERROR:',e.stack||e.message||e);throw e;}
