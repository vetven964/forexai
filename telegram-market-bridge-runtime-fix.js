...
const MARK='VTRADE_TELEGRAM_MARKET_BRIDGE_RUNTIME_FIX_V6';
...
function repairServer(){
  ...
  if(s!==before){fs.writeFileSync(SERVER,s,'utf8');console.log('[V-TRADE TELEGRAM BRIDGE FIX] repaired malformed V1 server injection | V6');}
}
...
function patchBridge(){
  ...
  const oldCanonStart=s.indexOf('function canonicalAuthority(d){');
  const oldCanonEnd=s.indexOf('async function fetchCanonical',oldCanonStart);
  if(oldCanonStart>=0&&oldCanonEnd>oldCanonStart)s=s.slice(0,oldCanonStart)+canonical+'\n'+s.slice(oldCanonEnd);

  const fetch=`async function fetchCanonical(port,path,headers){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);try{const base=String(process.env.VTRADE_CORE_URL||process.env.RENDER_EXTERNAL_URL||process.env.APP_BASE_URL||('http://127.0.0.1:'+port)).replace(/\\/$/,'');const r=await fetch(base+path,{headers:{...headers,'Cache-Control':'no-cache'},signal:ctl.signal});const d=await r.json().catch(()=>({success:false,error:'invalid-pre-market-json'}));return{r,d};}finally{clearTimeout(timer);}}`;
  const oldFetchStart=s.indexOf('async function fetchCanonical(port,path,headers){');
  const oldFetchEnd=s.indexOf('function scoreCandidate',oldFetchStart);
  if(oldFetchStart>=0&&oldFetchEnd>oldFetchStart)s=s.slice(0,oldFetchStart)+fetch+'\n'+s.slice(oldFetchEnd);
  ...
  if(s!==before){fs.writeFileSync(BRIDGE,s,'utf8');console.log('[V-TRADE TELEGRAM BRIDGE FIX] canonical MT5 authority adapter V6 installed');}
}
