...
function normalize(a){
 ...
 const authorized=signal!=='WAIT'&&decisionPassed&&newsPass&&brokerFresh&&dataOk&&a?.execution?.status!=='WAIT'&&n(a?.entry)!=null&&n(a?.stopLoss)!=null&&tp.length>=3;
 const gates={mtf:c.mtfAligned===true,sweep:c.liquiditySweep===true,mss:c.mss===true,bos:c.bos===true,displacement:c.displacement?.confirmed===true||c.displacement===true,fvg:c.freshFvg===true,ob:c.freshOb===true,pd:c.premiumDiscountOk===true,momentum:c.technicalMomentumOk===true,trend:c.trendStrengthOk===true,spread:c.spreadOk===true};
 return {a,signal:authorized?signal:'WAIT',rawSignal:signal,authorized,newsPass,brokerFresh,dataOk,qAge,gates,passed:Object.values(gates).filter(Boolean).length,total:Object.keys(gates).length};
}
function format(x,manual=false){const a=x.a,g=x.gates,side=x.signal==='BUY'?'🟢 BUY':x.signal==='SELL'?'🔴 SELL':'🟡 WAIT';const lines=['🤖 *V TRADE AI — XAUUSD*',`💰 ${F(a.livePrice??a.price)} | *${side}*`,`📈 *${a.bias||'NEUTRAL'}* | Score *${a.setupScore??a.confidence??0}/100* | Conf *${a.confidence??0}/100*`,`🔎 ICT *${x.passed}/${x.total}*`,`💧 Sweep ${yes(g.sweep)} | MSS ${yes(g.mss)} | BOS ${yes(g.bos)}`,`⚡ Disp ${yes(g.displacement)} | FVG ${yes(g.fvg)} | OB ${yes(g.ob)}`,`📍 MTF ${yes(g.mtf)} | PD ${yes(g.pd)} | Mom ${yes(g.momentum)} | ADX ${yes(g.trend)} | Spread ${yes(g.spread)}`,`📰 News ${x.newsPass?'PASS':'WAIT'}`,`🎯 Entry ${F(a.entry)} | SL ${F(a.stopLoss)}`,`🎯 TP1 ${F(a.takeProfit?.[0])} | TP2 ${F(a.takeProfit?.[1])} | TP3 ${F(a.takeProfit?.[2])}`,'',x.authorized?'🔐 *TRADE AUTHORIZED*':'⏳ *WAIT — NO ORDER*',`🧠 ${a.decision?.reason||a.trigger||'Mandatory confirmation gates are not complete.'}`];if(manual)lines.splice(3,0,'🖐️ *MANUAL STATUS REQUEST*');return lines.join('\n');}
async function scan(force=false,manual=false){if(busy)return;busy=true;try{const snap=await snapshot();const a=authorityFrom(snap);if(!a)throw new Error('Canonical authority payload missing');const x=normalize(a);console.log(`[V-TRADE TELEGRAM V6] Scan | raw=${x.rawSignal} | final=${x.signal} | bias=${a.bias||'NEUTRAL'} | score=${a.setupScore??a.confidence??0} | conf=${a.confidence??0} | gates=${x.passed}/${x.total} | authority=${x.authorized}`);if(!manual&&!x.authorized){console.log('[V-TRADE TELEGRAM V6] WAIT suppressed | canonical authority not authorized');return;}const key=x.authorized?`${x.signal}|${a.candleAgeSec}|${a.executionTimeframe}|${a.entry}|${a.stopLoss}|${(a.takeProfit||[]).join(',')}`:`MANUAL|${a.signal}|${a.timestamp}|${a.livePrice}`;if(!force&&key===lastSentKey)return;await bot.sendMessage(CHAT_ID,format(x,manual),{parse_mode:'Markdown'});lastSentKey=key;console.log(`[V-TRADE TELEGRAM V6] SENT | signal=${x.signal} | authorized=${x.authorized} | manual=${manual}`);}catch(e){console.warn('[V-TRADE TELEGRAM V6] delivery failed:',e.message);}finally{busy=false;}}
...
console.log('[V-TRADE TELEGRAM V6] CANONICAL AUTHORITY ONLY | CORE analysis owner | Telegram delivery owner | broker-native MT5 | fail-closed | WAIT auto-suppressed');
