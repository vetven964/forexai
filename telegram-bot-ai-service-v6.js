...
async function snapshot(){const d=await request('/api/telegram/market-snapshot');if(d.success!==true)throw new Error(d.error||'Telegram market snapshot unavailable');return d;}
async function core(path){return request(path);}
function normalize(a){
  ...
  const canonicalPassed=a?.decision?.passed===true||a?.confirmations?.allGatesPassed===true||a?.gates?.allGatesPassed===true;
  ...
  const authorized=signal!=='WAIT'&&canonicalPassed&&brokerFresh&&hasTradeLevels;
  ...
  const required=Number(a?.processing?.gatesRequired||10);
  ...
  return {a,signal:authorized?signal:'WAIT',rawSignal:signal,authorized,brokerFresh,canonicalPassed,gates,passed,total:required,reasons};
}
...
bot.onText(/^\/start(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;await sendText(msg,'🤖 *V TRADE AI — Telegram Independent V4.2*\n\n`/start` — Bot start\n`/help` — All commands\n`/signal` — XAUUSD ICT + CRT + Friday/Monday context\n`/price` — Broker-native MT5 price + MTF\n`/news` — Latest macro/news\n`/macro` — Macro radar\n`/fed` — Fed-focused news\n`/status` — Bot/system status');});
bot.onText(/^\/help(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;await sendText(msg,'🤖 *V TRADE AI — Telegram Independent V4.2*\n\n/start — Bot start\n/help — All commands\n/signal — XAUUSD ICT + CRT + Friday/Monday context\n/price — Broker-native MT5 price + MTF counts\n/news — Latest macro/news\n/macro — Macro radar\n/fed — Fed-focused news\n/status — Bot/system status');});
bot.onText(/^\/signal(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;await scan(true,true);});
bot.onText(/^\/price(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;try{const d=await snapshot();const a=authorityFrom(d)||{};const m=d.mtfCounts||d.mtf||{};await sendText(msg,`💰 *XAUUSD PRICE*\nPrice: *${F(d.price??a.livePrice??a.price)}*\nMT5: ${m.M5??'WAIT'} | ${m.M15??'WAIT'} | ${m.H1??'WAIT'} | ${m.H4??'WAIT'}\nState: *${d.state||'WAIT'}*\n${transitionLine(d)}`);}catch(e){await sendText(msg,`⚠️ *PRICE unavailable*\n${e.message}`);}});
