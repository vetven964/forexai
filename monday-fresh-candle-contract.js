/* V-TRADE AI — Monday fresh-candle execution contract V4 */
'use strict';
const fs=require('fs');
const path=require('path');
const SERVER=path.join(__dirname,'server.js');
const MARKER='VTRADE_MONDAY_FRESH_CANDLE_CONTRACT_V4';
...
function patch(source){
  ...
  const guarded=`const mondayFresh=typeof globalThis.vtradeMondayExecutionFreshTime==='function'?globalThis.vtradeMondayExecutionFreshTime(m?.candle?.candleTime,Date.now()):false;\n    const all=ready===4&&bias!=='NEUTRAL'&&liq&&mss&&bos&&displacement&&fvg&&ob&&pdOk&&!!execZone&&momentum&&spreadOk&&mondayFresh;`;
...
try{
  if(fs.existsSync(SERVER)){
    ...
    if(after!==before){fs.writeFileSync(SERVER,after,'utf8');console.log('[V-TRADE MONDAY] V4 active | Friday history retained | fresh Monday M5 required | fail-closed');}
  }
}catch(e){console.error('[V-TRADE MONDAY] V4 failed:',e.stack||e.message);throw e;}
