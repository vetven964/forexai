...
// TELEGRAM = independent child processes using canonical server signal authority.
const fs=require('fs');
...
const PREMARKET_TELEGRAM_SERVICE=path.join(ROOT,'telegram-premarket-signal-v2.js');
...
function installTelegramBridge(){
  const server=path.join(ROOT,'server.js');if(!fs.existsSync(server))throw new Error('server.js not found');
  let s=fs.readFileSync(server,'utf8');const anchor='const app = express();';const marker='VTRADE_TELEGRAM_SIGNAL_BRIDGE_V1';
  if(!s.includes(marker)){if(!s.includes(anchor))throw new Error('server app marker not found');const code=`\n/* ${marker} */\ntry{require('./telegram-signal-bridge.js').install(app);}catch(e){console.error('[V-TRADE TELEGRAM BRIDGE] install failed:',e.message);throw e;}\n`;s=s.replace(anchor,anchor+code);fs.writeFileSync(server,s,'utf8');console.log('[V-TRADE TELEGRAM BRIDGE] installed into CORE')}
}
function installNewsCalendarGate(){
  const server=path.join(ROOT,'server.js');if(!fs.existsSync(server))throw new Error('server.js not found');let s=fs.readFileSync(server,'utf8');
  if(s.includes(NEWS_GATE_MARK)){console.log('[V-TRADE NEWS V1] authoritative calendar gate already installed');return}
  const anchor='const app = express();';if(!s.includes(anchor))throw new Error('server app marker not found for news gate');
  const code=`\n/* ${NEWS_GATE_MARK} */\ntry{require('./pre-market-news-calendar-hotfix-v1')(app);}catch(e){console.error('[V-TRADE NEWS V1] install failed:',e.message);throw e;}\n`;s=s.replace(anchor,anchor+code,1);fs.writeFileSync(server,s,'utf8');console.log('[V-TRADE NEWS V1] authoritative economic calendar gate installed before CORE routes');
}
function startIndependentTelegram(){
  if(String(process.env.VTRADE_TELEGRAM_SEPARATE||'true').toLowerCase()!=='true'){console.log('[V-TRADE TELEGRAM AI] separate services disabled');return}
  const token=process.env.TELEGRAM_TOKEN||process.env.TELEGRAM_AUTO_TOKEN||'';const chat=process.env.TELEGRAM_CHAT_ID||process.env.TELEGRAM_AUTO_CHAT_ID||'';
  ...
  if(!fs.existsSync(PREMARKET_TELEGRAM_SERVICE))throw new Error('telegram-premarket-signal-v2.js not found');
  ...
  console.log('[V-TRADE TELEGRAM SEPARATION] Legacy CORE Telegram Auto Scanner = DISABLED');console.log('[V-TRADE TELEGRAM SEPARATION] Canonical Telegram Bot V6 = command/context service');console.log('[V-TRADE TELEGRAM SEPARATION] Pre-Market Signal V3 = signal-only consumer');
  const spawnChild=(file,label)=>{const child=spawn(process.execPath,[file],{env:childEnv,stdio:'inherit'});child.on('exit',(code,signal)=>console.warn(`[V-TRADE TELEGRAM ${label}] child exited | code=${code??''} signal=${signal||''}`));child.on('error',e=>console.error(`[V-TRADE TELEGRAM ${label}] child error:`,e.message));return child};
  setTimeout(()=>{if(global.__vtradeTelegramChild&&!global.__vtradeTelegramChild.killed)return;console.log('[V-TRADE TELEGRAM AI] delayed child boot | CORE startup grace complete');global.__vtradeTelegramChild=spawnChild(TELEGRAM_SERVICE,'V6')},8000);
  setTimeout(()=>{if(global.__vtradeTelegramPreMarketChild&&!global.__vtradeTelegramPreMarketChild.killed)return;console.log('[V-TRADE TELEGRAM AI] Pre-Market Signal child boot | canonical authority');global.__vtradeTelegramPreMarketChild=spawnChild(PREMARKET_TELEGRAM_SERVICE,'PREMARKET-V3')},9000);
  console.log('[V-TRADE PROCESS SEPARATION] CORE=PRE-MARKET/AI | TELEGRAM=V6 + PREMARKET-SIGNAL child | canonical authority');
}
...
installDashboardUI();installTelegramBridge();installNewsCalendarGate();
...
require('./pre-market-structure-hook.js');require('./predeploy-consistency-hotfix.js');require('./vtrade-start.js');
...
require('./telegram-single-renderer-lock.js');require('./server-launcher.js');startIndependentTelegram();
