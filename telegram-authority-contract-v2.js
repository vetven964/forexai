...
if(fs.existsSync(FILE)){
  ...
  // The bridge may already contain the newer canonical-authority implementation.
  // In that case this compatibility patch must be a no-op, not a startup failure.
  if(s.includes('function canonicalAuthority(')||s.includes('authorityContract:\'V1\'')){
    console.log('[V-TRADE TELEGRAM AUTHORITY] canonical authority already installed | compatibility patch skipped');
    ...
    if(s.includes(needle)){
      ...
      if(s.includes(fallback)){
        ...
        console.warn('[V-TRADE TELEGRAM AUTHORITY] authority anchor not found | leaving bridge unchanged');
      }
    }
