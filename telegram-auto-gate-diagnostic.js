...
function diagnose(a) {
  ...
  const gates = {
    'MSS/BOS': normalize(c.mssBos ?? c.mss ?? ict.mssBos ?? a.mssBos),
    'Liquidity': normalize(c.liquiditySweep ?? c.liquidity ?? ict.liquiditySweep ?? a.liquiditySweep),
    'FVG': normalize(c.fvg ?? ict.fvg ?? a.fvg), 'OB': normalize(c.ob ?? ict.ob ?? a.ob),
    'MTF': normalize(c.mtf ?? c.mtfAlignment ?? wf.mtf ?? a.mtf),
    'Entry Zone': normalize(c.entryZone ?? c.zone ?? wf.entryZone ?? a.entryZone),
    'RR': normalize(c.rr ?? c.riskReward ?? wf.riskReward ?? a.riskReward),
    'AI Confirm': normalize(c.aiConfirmation ?? c.aiConfirm ?? a.aiConfirmation ?? a.aiConfirm)
  };
