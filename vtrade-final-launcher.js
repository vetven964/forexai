// V-TRADE FINAL PRODUCTION LAUNCHER V15
...
try{
 validate();
 ownership();
 safeRequire('./vzone-runtime-compat.js','V-ZONE candleGuard compatibility binding');
 ...
 safeRequire('./vtrade-real-candle-telegram-bridge-hotfix.js','Canonical MT5 real-candle Telegram bridge');
 ...
 if(String(process.env.TELEGRAM_TOKEN||'').trim()&&String(process.env.TELEGRAM_CHAT_ID||'').trim()){
   ...
   console.log('[V-TRADE FINAL V15] Telegram result child isolated | CORE will remain alive on child failure');
 }else console.log('[V-TRADE FINAL V15] Telegram children skipped | credentials not configured');
 const core=safeRequire('./vtrade-enhanced-launcher.js','CORE launcher');
 if(!core) console.error('[V-TRADE FINAL V15] CORE launcher failed; no child failure may terminate this process');
}catch(e){
 console.error('[V-TRADE FINAL V15] STARTUP ERROR:',e.stack||e.message||e);
}
