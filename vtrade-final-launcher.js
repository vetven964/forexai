// V-TRADE FINAL PRODUCTION LAUNCHER V12
...
try{
 ...
 if(String(process.env.TELEGRAM_TOKEN||'').trim()&&String(process.env.TELEGRAM_CHAT_ID||'').trim()){
   ...
   console.log('[V-TRADE FINAL V12] Telegram result child isolated | CORE will remain alive on child failure');
 }else console.log('[V-TRADE FINAL V12] Telegram children skipped | credentials not configured');
 const core=safeRequire('./vtrade-enhanced-launcher.js','CORE launcher');
 if(!core) console.error('[V-TRADE FINAL V12] CORE launcher failed; no child failure may terminate this process');
}catch(e){
 console.error('[V-TRADE FINAL V12] STARTUP ERROR:',e.stack||e.message||e);
}
