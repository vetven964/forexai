    ...
    try{
      ...
      const barsFor=tf=>{
        try{
          ...
          return (Array.isArray(closed)?closed:[]).map(x=>({t:n(x?.t??x?.time??x?.timestamp??x?.timeMs),o:n(x?.o??x?.open),h:n(x?.h??x?.high),l:n(x?.l??x?.low),c:n(x?.c??x?.close),v:n(x?.v??x?.volume??x?.tickVolume)??0})).filter(x=>[x.o,x.h,x.l,x.c].every(Number.isFinite)).slice(-120);
        }catch(_){return [];}
      };
      const q=typeof brokerLivePrice==='function'?brokerLivePrice():null;
      const directPrice=validPrice(q?.price??q?.last??q?.close);
      const bid=validPrice(q?.bid),ask=validPrice(q?.ask);
      const mid=bid!=null&&ask!=null?(bid+ask)/2:null;
      const price=directPrice??validPrice(mid);
      const timeframes={};
      for(const tf of ['M5','M15','H1','H4']){const bars=barsFor(tf);timeframes[tf]={bars,count:bars.length,ready:bars.length>=30};}
      ...
      return res.json({success:true,source:'BROKER_NATIVE_MT5',contract:MARKER,symbol:'XAUUSD',price,bid,ask,spread,connected,quoteFresh:feedFresh,quoteAgeSec:age,state:connected?'READY':'WAIT',timeframes,generatedAt:new Date().toISOString(),telegramRole:'INDEPENDENT_AI_SCAN',preMarketLoaded:false,preMarketAuthority:false,executionLoaded:false});
    }catch(e){return res.status(502).json({success:false,error:String(e?.message||e),source:'BROKER_NATIVE_MT5'});}
  });
