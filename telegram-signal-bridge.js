/* V-TRADE AI — Telegram Market Bridge V6.2
 * READ-ONLY MT5 MARKET DATA ONLY.
 * Canonical Pre-Market V8 -> Telegram AI handoff.
 * Robustly unwraps canonical payloads so M5/M15/H1/H4 bars cannot be lost.
 * Monday execution remains fail-closed until a fresh Monday M5 candle exists.
 */
'use strict';
const MARKER='VTRADE_TELEGRAM_MARKET_BRIDGE_V6_2';
...
function install(app){
  if(!app||app.__VTRADE_TELEGRAM_MARKET_BRIDGE_V6_2__)return;
  app.__VTRADE_TELEGRAM_MARKET_BRIDGE_V6_2__=true;
  app.get('/api/telegram/market-snapshot',async(req,res)=>{
    ...
    try{
      const port=Number(process.env.PORT||10000),controller=new AbortController(),timer=setTimeout(()=>controller.abort(),12000);
      let upstream;
      ...
      console.log(`[V-TRADE TELEGRAM BRIDGE] SNAPSHOT | MTF=${Object.entries(mtfCounts).map(([tf,c])=>tf+':'+c).join(',')} | transition=${marketTransition?.phase||'UNKNOWN'} | freshMondayM5=${mondayFreshM5}`);
  ...
  console.log('[V-TRADE TELEGRAM BRIDGE] V6.2 ACTIVE | canonical V8 MTF unwrap + normalization | Friday historical -> Sunday pre-open -> Monday fresh M5 gate | fail-closed');
}
