...
function patch() {
  ...
  const waitSource = [
    ...
    "  return ['🤖 *V TRADE AI — XAUUSD*','',action,'💰 Price: *'+fmt(price)+'*','📈 Direction Score: *'+(Number.isFinite(score)?score:0)+'/100*','🧠 Confidence: *'+(Number.isFinite(confidence)?confidence:0)+'/100*','',gate('MSS/BOS',mss),gate('Liquidity',liquidity),gate('FVG/OB',fvg||ob),gate('MTF',mtf),'','🎯 Entry Zone: *'+zoneText+'*','🟢 Entry: *WAIT*','🛑 SL: *WAIT*','🎯 TP1: *WAIT*','🎯 TP2: *WAIT*','🎯 TP3: *WAIT*','📐 RR: *WAIT*','', '🤖 AI: *'+aiDecision+'* | '+(Number.isFinite(aiConfidence)?aiConfidence:0)+'/100 | '+agreement,'🔒 *WAIT — NO ORDER AUTHORIZED*','🏦 '+broker+' | Quote: '+age+'s'].join('\\n');",
    '}', ''
  ].join('\\n');
