/* V TRADE AI — Mobile bilingual UI V2
 * Phone-only language switcher: English / Khmer.
 * One owner for phone i18n. Trading calculations, MT5 feed, ICT gates and execution are untouched.
 */
(()=>{
'use strict';
if(window.__VTRADE_MOBILE_I18N_V2__)return;
window.__VTRADE_MOBILE_I18N_V2__=true;
...
function translateString(raw,kh){
 const source=String(raw??'');
 if(!kh)return source;
 let out=source;
 Object.keys(dict).sort((a,b)=>b.length-a.length).forEach(k=>{
   out=out.replace(new RegExp(escapeRx(k),'g'),dict[k]);
 });
 return out;
}
function walk(){
 ...
 while((n=walker.nextNode())){
   ...
   if(!p||['SCRIPT','STYLE','NOSCRIPT','TEXTAREA'].includes(p.tagName))continue;
   if(p.closest('#vtradePhoneLang'))continue;
   nodes.push(n);
 }
 const kh=localStorage.getItem(KEY)==='km';
 for(const node of nodes){
   if(!originals.has(node))originals.set(node,node.nodeValue||'');
   const raw=originals.get(node);if(!norm(raw))continue;
   const next=translateString(raw,kh);
   if(node.nodeValue!==next)node.nodeValue=next;
 }
 document.documentElement.lang=kh?'km':'en';
 document.documentElement.dataset.vtradeLang=kh?'km':'en';
 document.querySelectorAll('#vtradePhoneLang button').forEach(b=>{
   const active=b.dataset.lang===(kh?'km':'en');
   b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));
 });
}
function add(){
 if(!phone()||document.getElementById('vtradePhoneLang'))return;
 const host=document.createElement('div');host.id='vtradePhoneLang';
 host.innerHTML='<button type="button" data-lang="en">EN</button><button type="button" data-lang="km">ខ្មែរ</button>';
 ...
 host.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{localStorage.setItem(KEY,b.dataset.lang==='km'?'km':'en');walk();},true));
}
function safeArea(){
 if(!phone())return;
 let s=document.getElementById('vtradePhoneSafeArea');
 if(!s){s=document.createElement('style');s.id='vtradePhoneSafeArea';document.head.appendChild(s)}
 s.textContent='@media(max-width:900px){body.vtrade-mobile{padding-bottom:calc(154px + env(safe-area-inset-bottom))!important}body.vtrade-mobile .wrap{padding-bottom:154px!important;scroll-padding-bottom:154px!important}#vtradePreMarket{scroll-margin-bottom:154px!important}.footerline{padding-bottom:12px!important}}';
}
let scheduled=0;
