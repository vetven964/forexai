 ...
 * Market-data safety: never render or send a signal with an invalid/zero quote.
 */
'use strict';
...
}catch(e){console.error('[V-TRADE TELEGRAM AI] bot initialization failed:',e?.stack||e);process.exit(1);}
...
function validMarket(m){
  ...
  const ready=required.every(tf=>bars(m,tf).length>=20);
  const quoteFresh=m?.quoteFresh===true||m?.fresh===true||m?.state==='READY';
  return connected&&price!=null&&price>0&&ready&&quoteFresh;
}
function analyze(m,pre){
  ...
  const buy=num(pre?.buyStrengthPct??pre?.buyScore??pre?.buyPct);const sell=num(pre?.sellStrengthPct??pre?.sellScore??pre?.sellPct);
...
async function scan(sendWait=false){
  ...
  try{
    const [m,pre]=await Promise.all([getJSON('/api/telegram/market-snapshot'),getJSON('/api/pre-market/xauusd')]);
    if(!validMarket(m)){
      console.warn('[V-TRADE TELEGRAM AI] scan blocked | invalid/stale market snapshot | price='+String(m?.price??'null')+' | connected='+String(m?.connected??false)+' | state='+String(m?.state??'UNKNOWN'));
      return;
    }
    const a=analyze(m,pre);
    console.log(`[TELEGRAM AUTO] CORE scan | signal=${a.signal} | bias=${a.bias} | strength=${a.directionScore} | confidence=${a.confidence} | status=${a.authorized?'AUTHORIZED':'WAIT — NO ENTRY'} | transition=${a.transition} | fridayHistorical=${a.historicalFriday}`);
    if(!a.authorized&&!sendWait)return;
    const key=[a.signal,a.price,a.entry,a.stop,a.directionScore,a.confidence,a.transition].join('|');
    if(key===lastKey)return;
    await bot.sendMessage(CHAT_ID,formatSignal(a),{parse_mode:'Markdown'});lastKey=key;
  }catch(e){console.warn('[V-TRADE TELEGRAM AI] scan failed:',e.message);}finally{busy=false;}
}
...
bot.onText(/^\/price(?:@\w+)?$/i,async msg=>{if(String(msg.chat.id)!==CHAT_ID)return;try{const m=await getJSON('/api/telegram/market-snapshot');if(!validMarket(m)){await bot.sendMessage(msg.chat.id,'⚠️ *MT5 quote unavailable or stale — waiting for fresh market data.*',{parse_mode:'Markdown'});return;}await bot.sendMessage(msg.chat.id,`💰 *XAUUSD:* ${fmt(m.price)}\n📡 MT5: *READY*`,{parse_mode:'Markdown'});}catch(_){await bot.sendMessage(msg.chat.id,'⚠️ MT5 price unavailable');}});
