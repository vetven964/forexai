(() => {
  ...
  function pickNumber(...values) { for (const value of values) { const n = Number(value); if (Number.isFinite(n) && n > 0) return n; } return null; }
  ...
  function render(page='dashboard') { content.innerHTML=pages[page]?pages[page]():pages.dashboard(); document.querySelectorAll('.nav').forEach(b=>b.classList.toggle('active',b.dataset.page===page)); bindPage(page); closeDrawer(); refreshLivePrice(); }
  function bindPage(page){
    $('#toggleIct')?.addEventListener('click',()=>{const d=$('#ictDetails'),open=!d.hidden;d.hidden=open;$('#toggleIct').textContent=open?'Show':'Hide';});
    if(page!=='terminal')return;
    $('#toggleGuard')?.addEventListener('click',()=>{const d=$('#guardDetails'),open=!d.hidden;d.hidden=open;$('#toggleGuard').textContent=open?'Show':'Hide';});
  ...
  render('dashboard'); setInterval(refreshLivePrice,5000);
})();